...
class Graph:
    def __init__(self, connections: list[tuple[str, str]] | None = None) -> None:
        self.dep_graph: dict[str, set[str]] = defaultdict(set)
        self.usage_graph: dict[str, set[str]] = defaultdict(set)
        if connections:
            self.add_connections(connections)

    def add_connections(self, connections: list[tuple[str, str]]) -> None:
        # Add connections (list of tuple pairs) to graph
        # Assume each pair is directional from A -> B
        for node1, node2 in connections:
            self.add(node1, node2)

    def add(self, node1: str, node2: str) -> None:
